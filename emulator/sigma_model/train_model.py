...
def main():
    ...
    if not torch.load("emulator/sigma_model/checkpoints/dataset_split.pt"):
        ...
        torch.save({
            ...
            "test_ps2d":    test_ps2d,
            "test_xhi":     test_xhi,
        }, "emulator/sigma_model/checkpoints/dataset_split.pt")
    else:
    ...
    ps2d_pred, xhi_pred, ps2d_sigma_pred, xhi_sigma = run_inference(model, test_thetas)
    ps2d_true = test_ps2d.numpy()
    xhi_true  = test_xhi.numpy()
    ps2d_sigma_pred = ps2d_sigma_pred.numpy() 
    xhi_sigma = xhi_sigma.numpy() 
    ps2d_pred = ps2d_pred.numpy() 
    xhi_pred = xhi_pred.numpy() 
    print_metrics(ps2d_pred, ps2d_true, xhi_pred, xhi_true, ps2d_sigma = ps2d_sigma_pred, xhi_sigma = xhi_sigma)
    plot_pred_vs_true(ps2d_pred, ps2d_true, xhi_pred, xhi_true,out = "emulator/sigma_model/metrics/pred_vs_true.png")
    plot_residuals   (ps2d_pred, ps2d_true, xhi_pred, xhi_true,out = "emulator/sigma_model/metrics/residuals.png")
    plot_ps2d_maps   (ps2d_pred, ps2d_true, sample_idx=0, out = "emulator/sigma_model/metrics/ps2d_maps.png")
