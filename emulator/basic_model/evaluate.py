...
def plot_loss_history(history: dict, log_scale: bool = True):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("MSE Loss (log scale)" if log_scale else "MSE Loss")
    ax.set_title("Train vs Validation Loss")
    ax.legend()
    ax.grid(True, which="both", alpha=0.3)

    plt.tight_layout()
    plt.savefig("emulator/basic_model/metrics/loss_history.png", dpi=150)
    plt.show()
...
def plot_pred_vs_true(ps2d_pred, ps2d_true, xhi_pred, xhi_true, out="eval_scatter.png"):
    ...
    fig.tight_layout()
    fig.savefig("emulator/basic_model/metrics/" + out, dpi=150)
    plt.close(fig)
    print(f"  Saved → {out}")
...
def plot_ps2d_maps(ps2d_pred, ps2d_true, sample_idx=0, out="eval_ps2d_maps.png"):
    ...
    fig.savefig( "emulator/basic_model/metrics/" + out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved → {out}")
